 ...
 * \item u: Left singular vectors (J × K matrix, genes × factors)
 * \item v: Right singular vectors (N × K matrix, cells × factors)
 * }
 *
 * @keywords internal
 * @noRd
 ...
 *
 * Performs the GEDI 3-stage factorized SVD: SVD(Z) × SVD(middle) × SVD(projDB).
 ...
 *
 * @param Z Shared metagene matrix (J × K)
 * @param projDB Custom projected cell matrix (K × N_total), where N_total
 ...
 * \item d: Singular values (length K vector)
 * \item u: Left singular vectors (J × K matrix, genes × factors)
 * \item v: Right singular vectors (N_total × K matrix, custom_cells × factors)
 * }
 *
 * @keywords internal
 * @noRd
